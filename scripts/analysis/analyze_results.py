...
def write_csv(results):
    ...
    fields = [
        ...
        "total_healing_done", "total_errors", "timestamp", "_file"
    ]
...
def generate_report(results, stats):
    ...
    lines = []
    lines.append("# Balance Report — 120 Bot Playthroughs")
    lines.append("")
    ...
    # Bot limitation caveat
    lines.append("## Important Caveat")
    lines.append("")
    lines.append("The survival bot uses **only basic combat + heal + explore**. It does **NOT** use:")
    lines.append("- Stealth toggle or stealth-based strategies")
    lines.append("- Sustained songs (Freedom, Trees, Aule)")
    lines.append("- Voice abilities (Word of Command, Shout, etc.)")
    lines.append("- Forging/smithing at forge tiles")
    lines.append("- Archery or ranged combat")
    lines.append("- Equipment management (auto-equip only)")
    lines.append("")
    lines.append("Stat builds differ across archetypes but **strategies are identical**. "
                 "Data reflects baseline survivability, not optimal human play.")
    lines.append("")
