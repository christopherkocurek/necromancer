#!/usr/bin/env python3
"""
Necromancer Item Sprite Generator v3
Generates item sprites using DALL-E 3 with improved prompts and parallel generation.

Upgrades from item_gen.py:
- Green BG (#00FF00) default instead of magenta (with magenta fallback for green items)
- Anti-halo phrases ("plain flat background only, absolutely no other elements")
- Per-category framing directives (CATEGORY_FRAMING)
- ThreadPoolExecutor for parallel generation (max_workers=5)
- Saves RAW 1024x1024 only — processing done by process_items_v3.py

Usage:
    python item_gen_v3.py --all              # Generate all unique item sprites
    python item_gen_v3.py --category sword   # Generate one category
    python item_gen_v3.py --ids 64 67 68     # Generate specific item IDs
    python item_gen_v3.py --test             # 1 item per category for review
    python item_gen_v3.py --status           # Show progress
    python item_gen_v3.py --single 64        # Generate one item by ID
    python item_gen_v3.py --list             # List all items and categories
"""
...
# ============================================================================
# MAGENTA BG ITEMS - Green-colored items that need magenta BG instead of green
# ============================================================================

MAGENTA_BG_ITEMS = {
    # Green herbs
    383, 387, 389, 390, 404, 412, 413, 416,
    # Green potions
    321, 344,
    # Green jewelry
    135, 162, 171,
    ...
    # Green-brown armor
    23,
    # Brown wood items — invisible on green BG
    110,  # Silvan Bow
    71,   # Hunting Spear (brown shaft)
    86,   # Oak Staff (brown wood)
}

# ============================================================================
# CATEGORY FRAMING - Per-category composition directives for v3 prompts
# ============================================================================

CATEGORY_FRAMING = {
    "sword": "Lying diagonally, filling most of the frame. Thick blade with well-defined edges",
    "polearm": "Standing vertically or lying diagonally, filling most of the frame",
    "hafted": "Lying diagonally, heavy end prominent, filling most of the frame",
    "digging": "Lying diagonally, working end prominent, filling most of the frame",
    "bow": "Standing vertically, filling most of the frame",
    "sling": "Coiled neatly, filling most of the frame",
    "arrow": "Bundle lying diagonally, filling most of the frame",
    "sling_stone": "Small pile filling most of the frame",
    "body_armor": "Displayed front-facing, filling most of the frame",
    "soft_armor": "Displayed front-facing, filling most of the frame",
    "helm": "Seen from the front, filling most of the frame",
    "crown": "Floating slightly above center, filling most of the frame",
    "shield": "Displayed front-facing, slightly angled, filling most of the frame",
    "cloak": "Draped as if hanging, filling most of the frame",
    "boots": "Seen from the front, filling most of the frame",
    "gloves": "Displayed palms-down, filling most of the frame",
    "ring": "Enlarged detailed close-up filling 70% of the frame",
    "amulet": "Enlarged detailed close-up filling 70% of the frame",
    "potion": "Standing upright, thick opaque dark glass, filling most of the frame",
    "herb": "Seen from slightly above, filling most of the frame",
    "scroll": "Partially unrolled, filling most of the frame",
    "wand": "Lying diagonally, filling most of the frame",
    "horn": "Lying at a slight angle, filling most of the frame",
    "oil": "Standing upright, filling most of the frame",
    "light": "Standing upright, flame overlapping the shaft, filling most of the frame",
    "chest": "Front-facing, slightly open, filling most of the frame",
    "skeleton": "Seen from above, filling most of the frame",
    "special_material": "Filling most of the frame",
    "document": "Filling most of the frame",
}
...
CUSTOM_DESCS = {
    ...
    58: ("a single scimitar with one pale silvery-gray curved blade sweeping in a wide arc, brown leather-wrapped hilt with a polished steel disc pommel, one sword only one blade only, solid opaque blade with well-defined edges filling most of the frame",
         "one pale gray curved scimitar blade, brown leather hilt, steel pommel"),
    59: ("an elven sylvan blade with a long graceful scimitar curve widening to a heavier weighted tip, nut brown leather grip with a golden leaf pattern inlaid along the crossguard and pommel, solid opaque blade filling most of the frame",
         "curved scimitar shape, nut brown hilt, golden leaf inlay, heavier tip"),
    60: ("a crude heavy woodland blade like a long cleaver or blocky axe-sword, thick dark iron slab of a blade with rough hammer marks, simple wooden handle wrapped in rough hide, primitive and brutal, solid opaque blade filling most of the frame",
         "dark iron slab blade, rough hammer marks, wooden handle, primitive"),
    64: ("a German-style medieval longsword with a straight double-edged steel blade in soft warm light, long cruciform crossguard, pear-shaped pommel, dark brown leather-wrapped grip for hand-and-a-half use, clean balanced proportions, solid opaque blade filling most of the frame",
         "straight steel blade, soft warm light, cruciform crossguard, pear pommel, brown leather"),
    67: ("a Norse-inspired Rohirrim cavalry sword with a broad single-edged blade, five-lobed bronze pommel engraved with a horse head, short straight crossguard with gold knotwork, amber-brown leather grip, solid opaque blade with well-defined edges filling most of the frame",
         "broad blade, five-lobed bronze pommel, gold knotwork, Norse style"),
    68: ("a Numenorean hand-and-a-half bastard sword of bright polished steel, long straight double-edged blade with a subtle fuller, elegant silver crossguard with seven-star engravings, dark blue leather grip, solid opaque blade filling most of the frame",
         "bright polished steel, bastard sword, silver crossguard, seven stars, blue leather"),
    69: ("a single mithril sword with one solid opaque ice-blue blade radiating cold light, wide thick blade with pale frost patterns along the edge, silver crossguard and white leather grip, one sword only one blade only no crossed swords, solid opaque blade filling most of the frame",
         "one ice-blue blade only, frost patterns, cold light, silver crossguard, white leather"),
    ...
    491: ("a large broken magical sword with a wide cracked blade still pulsing with bright blue arcane energy, solid opaque fragments close together overlapping the hilt",
          "broken wide blade, bright blue arcane glow, dark steel"),
    ...
    71: ("a sturdy hunting spear with a large dark iron leaf-shaped spearhead, thick dark brown ash wood shaft wrapped in dark red leather grip, solid opaque shaft and head with well-defined edges filling most of the frame",
         "dark iron spearhead, dark brown shaft, dark red leather grip"),
    ...
    74: ("a terrifying Morgul glaive with a curved jet-black blade etched with glowing red cursed runes, black iron shaft with bone-white grip wrappings, solid opaque blade and shaft with well-defined edges filling most of the frame",
         "jet-black blade, glowing red runes, bone-white wrappings"),
    ...
    86: ("a heavy gnarled oak staff of dark rich brown wood with deep red-brown grain patterns, iron-banded ends with copper rivets, thick solid shaft filling most of the frame, solid opaque wood with well-defined edges",
         "dark rich brown wood, copper rivets, red-brown grain"),
    ...
    # === BOWS (tval=19) ===
    110: ("a thick solid silvan elf bow of dark brown wood with leaf-carved wide limbs, silver accents, wide solid bow arms filling the frame",
    ...
    251: ("a thick solid dark wooden flute with ornate silver keys and fairy runes, large close-up filling most of the frame",
          "dark brown wood, bright silver keys, fairy runes"),
    ...
    385: ("a large bundle of withered emptiness herb, thick desiccated gray-brown leaves and dry stems tied together, solid opaque bundle filling most of the frame",
          "desiccated gray-brown leaves, thick dry stems, dark twine"),
    ...
    425: ("a large thick bundle of dark brown thornvine roots with sharp thorns, solid opaque woody roots tied with dark twine, filling most of the frame",
          "dark brown roots, sharp thorns, thick solid bundle"),
    ...
    141: ("a large solid opaque crystal amulet of clear sight on a thick dark chain, bright blue crystal eye design with golden iris, solid opaque pendant filling most of the frame",
          "dark chain, bright blue crystal eye, golden iris, solid opaque"),
...
# ============================================================================
# DALL-E PROMPT GENERATION (v3 formula)
# ============================================================================

def get_item_prompt(item: Dict) -> str:
    """Build a DALL-E 3 prompt for an item sprite using v3 green-BG formula."""
    tval = item.get("tval", 0)
    category = CATEGORIES.get(tval, {})
    item_id = item.get("id", item.get("canonical_id", 0))

    if item_id in MAGENTA_BG_ITEMS:
        bg_color, bg_hex = "magenta", "FF00FF"
    else:
        bg_color, bg_hex = "green", "00FF00"

    cat_name = category.get("name", "unknown")
    framing = CATEGORY_FRAMING.get(cat_name, "Filling most of the frame")
    material = detect_material(item.get("name", ""))
    highlight = item.get("colors", "metallic")

    prompt = (
        f"On a plain flat solid bright {bg_color} background hex #{bg_hex}. "
        f"A dark fantasy illustration of {item['visual_desc']}. "
        f"{framing}. Solid opaque {material} with well-defined edges. "
        f"{highlight} highlights only, no {bg_color} tinting. "
        f"Plain flat {bg_color} background only, absolutely no other elements or shapes in the image. "
        f"A single item only, no hands, no characters. Clean illustrative painting style."
    )
    return prompt


# ============================================================================
# GENERATOR CLASS (v3: raw-only output + ThreadPoolExecutor)
# ============================================================================
...
class ItemGenerator:
    ...
    def generate_single(self, item_id: int, retry: int = 0) -> Optional[Path]:
        """Generate a single item sprite. Saves RAW 1024x1024 only."""
        ...
        with self._lock:
            if key in self.progress["sprites"]:
                ...
                if info.get("status") == "completed":
                    raw_path = Path(info.get("raw_path", ""))
                    if raw_path.exists():
        ...
        try:
            ...
            # Save raw 1024x1024 only
            raw_dir = self.output_dir / "raw"
            raw_path = raw_dir / f"{key}_1024.png"
            img_1024.save(raw_path)
            print(f"  Saved raw: {raw_path.name}")

            # Determine BG color used for this item
            actual_item_id = item.get("id", item.get("canonical_id", 0))
            bg_used = "magenta" if actual_item_id in MAGENTA_BG_ITEMS else "green"
    ...
    def _generate_parallel(self, item_ids: List[int], label: str, max_workers: int = 5) -> Dict[str, bool]:
        """Generate multiple items in parallel using ThreadPoolExecutor."""
        results = {}

        print(f"\n{'=' * 60}")
        print(f"{label}")
        print(f"Items: {len(item_ids)} | Workers: {max_workers} | Est. cost: ${len(item_ids) * COST_PER_IMAGE:.2f}")
        print(f"{'=' * 60}")

        def _worker(item_id: int) -> Tuple[str, bool]:
            key = self._sprite_key(item_id)
            path = self.generate_single(item_id)
            time.sleep(RATE_LIMIT_DELAY)
            return key, path is not None
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    key = self._sprite_key(item_id)
                    results[key] = False
                    print(f"  THREAD ERROR for item {item_id}: {e}")
    ...
    def generate_category(self, category_name: str) -> Dict[str, bool]:
        ...
        cat_items = sorted(iid for iid, i in unique.items() if i.get("tval") == target_tval)

        cat = CATEGORIES[target_tval]
        results = self._generate_parallel(
            cat_items,
            f"CATEGORY: {cat['name'].upper()} (tval={target_tval})",
        )

        self._print_summary(results)
        return results

    def generate_ids(self, item_ids: List[int]) -> Dict[str, bool]:
        """Generate specific items by ID list."""
        # Validate IDs
        valid_ids = []
        for iid in item_ids:
            if iid in self.items:
                ...
                print(f"  WARNING: Unknown item ID {iid}, skipping")

        if not valid_ids:
            print("No valid item IDs to generate.")
            return {}

        results = self._generate_parallel(
            valid_ids,
            f"GENERATING {len(valid_ids)} SPECIFIC ITEMS",
        )
    ...
    def generate_test(self) -> Dict[str, bool]:
        ...
        results = self._generate_parallel(
            test_ids,
            f"ITEM TEST GENERATION - 1 per category ({len(test_ids)} sprites)",
        )
    ...
    def generate_all(self) -> Dict[str, bool]:
        ...
        for cat_name in sorted(by_cat.keys()):
            ...
            results = self._generate_parallel(
                ids,
                f"--- {cat_name.upper()} ({len(ids)} items) ---",
            )
    ...
    def print_status(self):
        ...
        print(f"\n{'=' * 60}")
        print(f"ITEM SPRITE GENERATION STATUS (v3)")
        print(f"{'=' * 60}")
    ...
    def list_items(self):
        ...
        for cat_name in sorted(by_cat.keys()):
            ...
            for item_id, item in items_list:
                ...
                magenta = " [M]" if item_id in MAGENTA_BG_ITEMS else ""
                print(f"  {item_id:4d}  {name:<35s}{dupe_str}{custom}{magenta}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate item sprites via DALL-E 3 (v3 - green BG, parallel)")
    ...
    group.add_argument("--ids", type=int, nargs="+", help="Generate specific item IDs")
